...
6. sub_tags：细分子标签数组，**最多 8 个**，按 "主分类 > 子领域 > 流派" 层级用 ">" 拼接（最多 3 层）。例：
   - psychology > psychoanalysis > object-relations
   - philosophy > phenomenology > husserl
   - mathematics > topology > algebraic-topology
   - history > modern-china > late-qing
7. language：内容语言代码（en / zh-CN / zh-TW / ja / de / fr / ru / la / grc / sa / ko / es / pt / ar 等 ISO 639）

**严格要求**：
- 只能输出 JSON，**不要任何解释、注释、markdown 围栏**
- 字段缺失就填空字符串或空数组，不要瞎猜
- sub_tags 必须小写、用 "-" 替代空格、只允许 a-z 0-9 - > 字符
...
按以下 JSON schema 严格输出（**只输出一个 JSON 对象**）：
{
  ...
  "sub_tags": ["philosophy > phenomenology > husserl"],
  "language": "en"
}`;
