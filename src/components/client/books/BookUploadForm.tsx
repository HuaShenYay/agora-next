...
export default function BookUploadForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append("titleOriginal", titleOriginal || title);
      ...
      if (res.ok) {
        setResult({ success: true, bookId: data.bookId });
      } else {
      ...
      >
        ...
        {file ? (
          ...
          <div className="upload-dropzone-hint">
            ...
            <p>拖拽文件到此处，或点击选择</p>
            <span className="upload-drop-formats">支持 PDF、EPUB、TXT、Markdown（≤50MB）</span>
          </div>
        )}
      </div>
      <div className="upload-fields">
        <div className="form-row">
          <div className="form-group">
            <label>书名 *</label>
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="如：理想国" required />
          </div>
          <div className="form-group">
            <label>原文书名</label>
            <input type="text" value={titleOriginal} onChange={(e) => setTitleOriginal(e.target.value)} placeholder="如：The Republic" />
          </div>
        </div>
        <div className="form-row">
          <div className="form-group">
            <label>作者 *</label>
            <input type="text" value={author} onChange={(e) => setAuthor(e.target.value)} placeholder="如：柏拉图 / Plato" required />
          </div>
          <div className="form-group">
            <label>原文语言</label>
